...
// TODO: aggiungere controllo dimensione LDROM e APROM
...
void usage() {
  ...
  fputs("  -p <port>\tserial port (if omitted it will be automatically "
        "selected)\n",
        stderr);
  exit(1);
}
...
void readAPROM(const char *filename) { readROM(filename, 'A'); }

void readLDROM(const char *filename) { readROM(filename, 'L'); }

void writeAPROM(const char *filename) { writeROM(filename, 'A'); }

void writeLDROM(const char *filename) { writeROM(filename, 'L'); }
...
int main(int argc, char *argv[]) {
  ...
  clock_t begin = clock();
  while ((opt = getopt_long(argc, argv, "p:r:w:x", long_options, &opt_index)) !=
         -1) {
    switch (opt) {
    case 'p':
